...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query("SELECT p, c.category_name FROM post p INNER JOIN category c ON p.category_id WHERE c.category_name = ?1")
    public List<Post> searchByCategory(String category);

    @Query("SELECT p, c.category_name FROM post p INNER JOIN category c ON p.category_id WHERE p.title = ?1 AND c.category_name = ?1")
